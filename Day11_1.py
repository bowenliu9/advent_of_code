...
def print_neighbors(seats, r,c):
    # three cases, corner, edge, middle
    row = len(seats)
    column = len(seats[0])
    sequence = [-1,0,1]
    for row_change in sequence:
        for column_change in sequence:
            r_index = r+row_change
            c_index = c+column_change
            if r_index>=0 and r_index<row and c_index>=0 and c_index<column:
                if row_change == 0 and column_change == 0:
                    pass
                else:
                    print("At ", r_index, c_index, "it is ", seats[r_index][c_index])
        
def should_occupy(seats, r,c):
    row = len(seats)
    column = len(seats[0])
    sequence = [-1,0,1]
    seat_nums = 0
        ...
        for column_change in sequence:
            ...
            if r_index>=0 and r_index<row and c_index>=0 and c_index<column:
                if row_change == 0 and column_change == 0:
                    ...
                    seat_nums+=1
                    if seats[r_index][c_index] != "#":
                        empty_nums+=1
    return seat_nums == empty_nums

def should_leave(seats, r, c):
    row = len(seats)
    column = len(seats[0])
    sequence = [-1,0,1]
    taken_nums = 0
    ...
    for r in range(len(seats)):
        ...
        for c in range(len(seats[0])):
            if seats[r][c] == ".":
                ...
                continue
            if seats[r][c] == "L":
                if should_occupy(seats,r,c):
                    # change L to #
                    b[r] += "#"
                else:
                    b[r] += "L"
            if seats[r][c] == "#":
                if should_leave(seats,r,c):
                    # change # to L
                    b[r] += "L"
                else:
                    b[r] += "#"
