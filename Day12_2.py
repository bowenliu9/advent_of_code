...
for command in commands:
    ...
    if action == "F":
        east_west += magnitude*waypoint[0]
        north_south += magnitude*waypoint[1]
    if action in "NSWE":
        if action == "N":
            waypoint[1] = waypoint[1]+magnitude
        if action == "S":
            waypoint[1] = waypoint[1]-magnitude
        if action == "W":
            waypoint[0] = waypoint[0]-magnitude
        if action == "E":
            waypoint[0] = waypoint[0]+magnitude
    if action in "LR":
